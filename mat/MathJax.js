	...
	if (!MathJax.Hub) {
			...
			b.Object = c({
				...
				Augment : function(g, h) {
					...
					if (g != null) {
						...
						if (g.toString !== this.prototype.toString
								&& g.toString !== {}.toString) {
				...
				SimpleSUPER : c({
					...
					wrapper : function() {
						...
						delete this.SUPER;
						return g
					}.toString().replace(/^\s*function\s*\(\)\s*\{\s*/i, "")
							.replace(/\s*\}\s*$/i, ""),
					toString : function() {
						return this.original.toString.apply(this.original,
								arguments)
					}
				})
			})
			...
			a.Ajax = {
				...
				timer : {
					create : function(i, h) {
						...
						if (h.nodeName === "STYLE"
								&& h.styleSheet
								&& typeof (h.styleSheet.cssText) !== "undefined") {
							i(this.STATUS.OK)
						} else {
							if (window.chrome
								...
								if (c) {
									this.timer.start(this, [
											this.timer.checkSafari2, f++, i ],
											this.styleDelay)
								} else {
									this.timer.start(this, [
											this.timer.checkLength, h, i ],
											this.styleDelay)
								}
							}
						}
					...
					checkSafari2 : function(h, i, j) {
						...
						if (document.styleSheets.length > i
				...
				loadError : function(h) {
					a.Message.Set(
							[ "LoadFailed", "File failed to load: %1", h ],
							null, 2000);
				...
				for ( var d = 0, a = c.elements.length; d < a; d++) {
					if (c.elements[d]) {
						b.Push([ "Post", this.signal,
								[ "Begin PreProcess", c.elements[d] ] ],
								(arguments.callee.disabled ? {} : [ "Execute",
										this.preProcessors, c.elements[d] ]), [
										"Post", this.signal,
										[ "End PreProcess", c.elements[d] ] ])
					}
				}
				return b.Push(c.callback)
			},
				...
				for ( var d = 0, a = c.elements.length; d < a; d++) {
					if (c.elements[d]) {
						...
						b
								.Push(
										[ "Post", this.signal,
												[ "Begin " + g, c.elements[d] ] ],
										[
												"Post",
												this.signal,
												[ "Begin Math", c.elements[d],
														g ] ],
										[ "prepareScripts", this, g,
												c.elements[d], f ],
										[
												"Post",
												this.signal,
		...
		MathJax.Hub.Startup = {
			...
			Config : function() {
				...
				if (b.URL || b.Config) {
					if (confirm(MathJax.Localization
							._(
									"CookieConfig",
		...
		(function(l) {
			...
			var a = {
				...
				isFirefox : (navigator.userAgent.match(/Gecko/) != null && navigator.userAgent
						.match(/KHTML/) == null),
				...
				isKonqueror : (window.hasOwnProperty
						&& window.hasOwnProperty("konqueror") && navigator.vendor == "KDE"),
			...
			c.Browser
					.Select({
						...
						Firefox : function(j) {
							if ((j.version === "0.0" || navigator.userAgent
									.match(/Firefox/) == null)
							...
							j.isMobile = (navigator.appVersion
									...
									|| navigator.userAgent.match(/ Fennec\//) != null || navigator.userAgent
									.match(/Mobile/) != null)
						},
						Opera : function(i) {
			...
			c.queue.Push([ "Post", q.signal, "Begin" ], [ "Config", q ], [
					"Cookie", q ], [ "Styles", q ], [ "Message", q ],
					function() {
						var i = f.Callback.Queue(q.Jax(), q.Extensions());
						return i.Push({})
					}, [ "Menu", q ], q.onLoad(), function() {
						MathJax.isReady = true
					}, [ "Typeset", q ], [ "Hash", q ], [ "MenuZoom", q ], [
							"Post", q.signal, "End" ])
		})("MathJax")
	}
};
